...
public class SuperWiFi extends MainActivity{
	...
	public void set_para(int testtime,int timeinterval){
		...
		AP_LIST=new String[1200];
		AP_LIST2=new String[1200];
		 AP_scanned_flag=new int[1200];
		 AP_ONTO_LOCATION=new int[1200];
		AP_SCAN_HISTORY=new int[1200][1200];
		APRSS=new int[1200];
		TESTTIME=testtime;
		TESTTIMENOW=0;
	...
	public boolean setNetwork(String name)
	{
		...
		try
		{
			...
			while(it.hasNext())
			{
				...
				if(network.BSSID.equals(name))//�ҵ�ָ��SSID������󣬶Ͽ������磬������
				{
					wm.disconnect();
...
//					}
				...
				for(int i=1;i<=AP_NUM+1;i++){
					//if (AP_LIST2[i-1].equals("IIoT-445-test") ||AP_LIST2[i-1].equals("IIoT-AlumniCafe-2.4G") ||AP_LIST2[i-1].equals("NETGEAR27") || AP_LIST2[i-1].equals("NETGEAR34")|| AP_LIST2[i-1].equals("NETGEAR11")|| AP_LIST2[i-1].equals("NETGEAR25")|| AP_LIST2[i-1].equals("NETGEAR24")) {
						scanned.add(
	...
	private void performScan()
	{
		...
		try
		{
			...
			try {
				Thread.sleep(TIME_INTERVAL);//ÿTIME_INTERVALɨ��һ�Σ�ֱ������
			} catch (InterruptedException e) {
				// TODO Auto-generated catch block
				e.printStackTrace();
			}
			...
			while(it.hasNext())
			{
				...
				if(flag==0){
				AP_NUM++;
				AP_LIST[AP_NUM]=ap.BSSID;
				AP_LIST2[AP_NUM]=ap.SSID;
